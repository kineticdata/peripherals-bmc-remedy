...
class BmcItsm7ChangeCreateV3
  ...
  # entries in the Changes form in ITSM.
  #
  # This is a required method that is automatically called by the Kinetic Task
  # Engine.
  #
  ...
  def execute()
    ...
    entry = @@remedy_forms['CHG:ChangeInterface_Create'].create_entry!(
      ...
      :fields       => ['Infrastructure Change Id', 'InstanceId', 'SRMSAOIGuid']
    )
    ...
    <results>
	  ...
	  <result name="Deferral Token">#{escape(entry['SRMSAOIGuid'])}</result>
    </results>
    RESULTS
