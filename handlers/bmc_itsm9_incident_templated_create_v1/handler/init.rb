...
class BmcItsm9IncidentTemplatedCreateV1
  ...
  def execute
	...
	@fields['TemplateID'] = get_template_id(@parameters['incident_template_name'], token)
    ...
    api_route = "#{@api_server}/arsys/v1/entry/HPD:IncidentInterface_Create/"
    ...
    incidentNumber = createdIncident_parsed["values"]["Incident Number"]  
	incidentId = createdIncident_parsed["values"]["InstanceId"]
	deferralToken = createdIncident_parsed["values"]["SRMSAOIGuid"]
	puts("Incident Number: #{incidentNumber}") if @debug_logging_enabled
	puts("Incident Instance Id: #{incidentId}") if @debug_logging_enabled
	puts("Deferral Token: #{deferralToken}") if @debug_logging_enabled
	
    # Build the results to be returned by this handler
    @results = "<results>"
	@results += "<result name='Handler Error Message'></result>"
	@results += "<result name='Incident Number'>#{escape(incidentNumber)}</result>"
	@results += "<result name='Incident Instance Id'>#{escape(incidentId)}</result>"
    ...
    @results += "</results>"
      ...
      if @error_handling == "Raise Error"
        ...
        @results = "<results>"
		@results += "<result name='Incident Number'></result>"
		@results += "<result name='Incident Instance Id'></result>"
  ...
  #
  def get_template_id(incident_template_name, token)
    puts('getting template id String') if @debug_logging_enabled
	query = URI.encode("'Template Name'=\"#{incident_template_name}\"")
	route = @api_server + '/arsys/v1/entry/HPD:Template?q='+query;
