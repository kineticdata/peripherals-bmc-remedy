...
class BmcItsm7IncidentStatusUpdateV2
  ...
  def initialize(input)
	...
	@IncidentID = get_field_value(@input_document, 'IncidentID')
	#puts @IncidentID
	...
	@field_values = {}
	@field_values['Status'] = get_field_value(@input_document, 'Status') 
	@field_values['Status_Reason'] = get_field_value(@input_document, 'Status_Reason') 
	#puts @field_values
  ...
  def execute()
    ...
    entry = @@remedy_form.find_entries(
	  ...
	  :conditions => [%|'1000000161'="#{@IncidentID}"|]
    )

	raise "Unable to retrieve record for id #{@IncidentID}" if entry.nil?
