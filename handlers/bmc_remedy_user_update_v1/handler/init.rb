...
class BmcRemedyUserUpdateV1
  ...
  def execute()
	...
	query = "'Login Name' = \""+@parameters["login_id"]+"\""
    api_route = "#{api_server}/arsys/v1/entry/User?q="+URI.encode(query)
    ...
    if token.length == 3  # for example 401 404 500...
      ...
      user_reponse = RestClient::Request.new({
	    ...
	    if response.code == 200
		  ...
		  if user_parsed["entries"][0].nil?
            ...
            requestId = user_parsed["entries"][0]["values"]["Request ID"] 
	        puts("Request ID: #{requestId}") if @debug_logging_enabled
	      end
	    else
          if @error_handling == "Raise Error"
	        raise result.body
	  ...
	  if error_message==""
	    ...
	    update_route = "#{api_server}/arsys/v1/entry/User/"+requestId
    ...
    RESULTS
    puts("Results: \n#{escape(results)}") if @debug_logging_enabled
	  return results
  end
