...
class BmcItsm7WorkOrderCreateDetailV2
  ...
  # Creates a WOI:Work Order record using the WOI:WorkOrderInterface_Create form.
  # The WOI:WorkOrderInterface_Create is used as a staging form form when raising 
  # entries in the Work Order form in ITSM.
  #
  # This is a required method that is automatically called by the Kinetic Task
  # Engine.
  #
  ...
  def execute()
    ...
    # that was built up.  Pass 'Work Order Number' and 'InstanceId' to the fields
    ...
    entry = @@remedy_forms['WOI:WorkOrderInterface_Create'].create_entry!(
      ...
      :fields       => ['WorkOrder_ID', 'InstanceId']
    )
    ...
    <results>
      ...
      <result name="Work Order Instance Id">#{escape(entry['InstanceId'])}</result>
    </results>
    RESULTS
