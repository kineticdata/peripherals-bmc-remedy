...
class BmcItsm8AssetToContractAssocCreateV1
  ...
  def execute()
	...
	entry = @@remedy_forms['CTR:Contract_Relationship'].create_entry!(
    ...
    <<-RESULTS
    <results>
      <result name="CTR:Contract_Relationship Entry ID">#{escape(entry.id)}</result>
    </results>
    RESULTS
  end
