...
class BmcItsm9SupportGroupMemberFunctionalRoleAddV1
  ...
  def execute()
    ...
    if token.length == 3  # for example 401 404 500...
	  ...
	  user_reponse = RestClient::Request.new({
	    ...
	    if response.code == 200
		  ...
		  if user_parsed["entries"][0].nil?
			...
			@field_values['Person ID'] = user_parsed["entries"][0]["values"]["Person ID"]
	        @field_values['Full Name'] = user_parsed["entries"][0]["values"]["Full Name"]
	      end
	    else
		  puts (response.body)
          if @error_handling == "Raise Error"
      ...
      user_reponse = RestClient::Request.new({
	    ...
	    if response.code == 200
		  ...
		  if support_parsed["entries"][0].nil?
            ...
            @field_values['Support Group ID'] = support_parsed["entries"][0]["values"]["Support Group ID"] 
	  ...
	  create_route = "#{api_server}/arsys/v1/entry/CTM:SupportGroupFunctionalRole"
	  puts("UPDATE ROUTE: #{create_route}") if @debug_logging_enabled
      ...
      response = RestClient::Request.new({
		...
		if response.code == 201
          ...
          newitem_request_id = newitem_parsed["values"]["SupportGroupFunctionalRole ID"]  
    ...
    <results>
      ...
      <result name="Result">#{results}</result>
	  <result name="Support Group Functional Role ID">#{newitem_request_id}</result>
    </results>
    RESULTS
