...
class RemedyGroupListUsersV1
  ...
  def execute()
    ...
    group = retrieve_group(group_name,["Group ID"])
    group_id = group["Group ID"].to_s
	    
    user_list = Array.new
    
    user_list = retrieve_users(group_id,["Login Name","Group List"])
    
    xml = '<users>'
    
    user_list.each do |user|
        
        if (user["Group List"].split(/;/).member?(group_id))
          xml << "<user>" << user["Login Name"] << "</user>"
        end
    ...
    <<-RESULTS
      <results>
          <result name="User List">#{escape(xml)}</result>
      </results>
    RESULTS
...
# TODO -- Needs updating to be accurate
  ...
  def retrieve_users(group_id, field_identifiers=[])
    entry = @@remedy_forms['User'].find_entries(
      :all, :fields => field_identifiers, :conditions => [%`'Group List' LIKE "%#{group_id}%"`]
    )
