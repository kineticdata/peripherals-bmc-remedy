...
class RemedyGenericQueryRetrieveV4
  ...
  def execute()
    ...
    return_type = @parameters["return_type"].downcase
    if return_type != "single" && return_type != "all"
      return_type = "single"
    end
    error_message = nil
    results_list = ""
    ...
    if (@error.to_s.empty?)
      begin
        ...
        if return_type == "single"
          ...
          puts("retrieving single record") if @debug_logging_enabled
        ...
        @entry = get_remedy_form(@parameters['form']).find_entries(
          ...
          puts("Parameters: #{@entry.inspect}") if @debug_logging_enabled
    ...
    puts("retrieving all records") if @debug_logging_enabled
    condition = %|#{@parameters['request_query']}|
      puts("Query: #{@parameters['request_query']}") if @debug_logging_enabled
  ...
  if @entry.kind_of?(Array)
    ...
    @entry.each do |entry_values|
      field_object = {}
    field_values = entry_values.field_values.collect do |field_id, value|
      "#{get_remedy_form(@parameters['form']).field_for(field_id).name}: #{value}"
      ...
      #Build result XML
      field_object[escape(get_remedy_form(@parameters['form']).field_for(field_id).name)] = escape(textvalue)
    end
    puts("Field Values: #{field_values.inspect}") if @debug_logging_enabled
    puts("Field Output: \n#{field_object.inspect}") if @debug_logging_enabled
    @result_entry.push(field_object)
  end
end
...
<result name="Results">#{escape(@result_entry.to_json)}</result>
</results>
RESULTS
  end
