...
class BmcItsm7IncidentTemplatedCreateDetailV4
  ...
  def initialize(input)
    ...
    preinitialize_on_first_load(
      @input_document,
      ['HPD:IncidentInterface_Create', 'HPD:Template', 'HPD:Help Desk']
    )
  ...
  # entries in the Incidents form in ITSM.  Before creating the record on the
  # HPD:IncidentInterface_Create form, it retrieves the 'InstanceId' of the given
  # incident template using the get_template_id() method.
  #
  # This is a required method that is automatically called by the Kinetic Task
  # Engine.
  #
  ...
  def execute()
    ...
    # to get_template_id().
    @field_values['TemplateID'] = get_template_id(@parameters['incident_template_name'])
    ...
    entry2 = @@remedy_forms['HPD:Help Desk'].find_entries(
      :first,
      :conditions => [%|'InstanceId' = "#{ entry['InstanceId'] }"|],
      :fields     => ['Status', 'Status_Reason', 'Resolution', 'Last Modified Date']
    )
 # Raise an exception if no HPD:Help Desk entry was found with the given Instance ID
#    if entry2.nil?
#      raise("Could not find status of HPD:Help Desk with ID: #{ entry['InstanceId'] } ") 
#     end

	if entry2['Status_Reason'].nil?
		statusreason = entry2['Status_Reason']
	else 
	    statusreason = entry2['Status_Reason'].value
	end
	
	#puts ("Status Results: \n")
    #puts(format_hash("header", @entry2)) #if @debug_logging_enabled
    ...
    <results>
      ...
      <result name="Status">#{escape(entry2['Status'].value)}</result>
	  <result name="Status_Reason">#{escape(statusreason)}</result>
	  <result name="Resolution">#{escape(entry2['Resolution'])}</result>
	  <result name="LMD">#{escape(entry2['Last Modified Date'])}</result>
     </results>
    RESULTS
 ...
 #puts("Results: \n#{results}")
  ...
  ##############################################################################
  # BMC_ITSM7_Incident_TemplatedCreate handler utility functions
  ##############################################################################

  # Retrieves the String 'InstanceId' of the HPD:Template record that is associated
  # to the specified incident template name.
  #
  # Raises an exception if there was no HPD:Template record that had 'Template Name'
  # equal to the specified incident_template_name parameter.
  #
  # ==== Parameters
  # * incident_template_name (String) - The name of the desired incident template.
  #
  def get_template_id(incident_template_name)
    # Retrieve the HPD:Template entry using the incident_template_name parameter
    # in the qualification.
    entry = @@remedy_forms['HPD:Template'].find_entries(
      :first,
      :conditions => [%|'Template Name' = "#{incident_template_name}" AND 'Status Template' = "Enabled"|],
      :fields     => ['InstanceId']
    )
    # Raise an exception if no HPD:Template entry was found with the given name.
    if entry.nil?
      raise("Could not find HPD:Template with name: #{incident_template_name}")
    end
    # Return the 'InstanceId' of the HPD:Template entry.
    entry['InstanceId']
  end
