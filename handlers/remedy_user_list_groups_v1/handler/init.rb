...
#    group_id = group["Group ID"].to_s
    ...
    group_list_name = Array.new

    unless (user["Group List"].nil?)
  	  group_list = user["Group List"].gsub(/\s/,"").split(/;/)

      group_list.each do |group_id|
        group_name_record = retrieve_group(group_id,["Group Name"])
        group_name = group_name_record["Group Name"]
        group_list_name << group_name
      end

    end

	  puts "Group list #{group_list_name.inspect}"

    xml = '<groups>'
    group_list_name.each do |group_name|
      xml << "<group>" << group_name << "</group>"
    end 
    xml << "</groups>"

    # This returns the RESULTS string
    # Build the results XML that will be returned by this handler.
    <<-RESULTS
      <results>
          <result name="Group List">#{escape(xml)}</result>
      </results>
    RESULTS
...
# TODO -- Needs updating to be accurate
  ...
  # An ArsModels::Entry record that includes the field values of the located
  # in Group record
  def retrieve_group(group_id, field_identifiers=[])
    entry = @@remedy_forms['Group'].find_entries(
      :single, :fields => field_identifiers, :conditions => [%`'Group ID' = #{group_id}`]
    )
    raise %|Unable to find the Group record for '#{group_id}'.| if entry.nil?
    entry
  end
