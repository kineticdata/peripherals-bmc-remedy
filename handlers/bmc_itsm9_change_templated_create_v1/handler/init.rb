...
class BmcItsm9ChangeTemplatedCreateV1
  ...
  def execute
	...
	@fields['TemplateID'] = get_template_id(@parameters['change_template_name'], token)
    puts(format_hash("Field Values:", @fields)) if @debug_logging_enabled
    ...
    api_route = "#{@api_server}/arsys/v1/entry/CHG:ChangeInterface_Create/"
	...
	createdChange_route = api_route + returnedId
	createdChange_resource = RestClient::Resource.new(createdChange_route)
	createdChange_reponse = createdChange_resource.get(:authorization => "AR-JWT "+token)
	createdChange_parsed = JSON.parse(createdChange_reponse)
	puts("createdChange_reponse: "+ createdChange_reponse) if @debug_logging_enabled

    changeNumber = createdChange_parsed["values"]["Infrastructure Change Id"]  
	changeId = createdChange_parsed["values"]["InstanceId"]
	deferralToken = createdChange_parsed["values"]["SRMSAOIGuid"]
	puts("Change Number: #{changeNumber}") if @debug_logging_enabled
	puts("Change Instance Id: #{changeId}") if @debug_logging_enabled
	puts("Deferral Token: #{deferralToken}") if @debug_logging_enabled
	
    # Build the results to be returned by this handler
    @results = "<results>"
	@results += "<result name='Handler Error Message'></result>"
	@results += "<result name='Change Number'>#{escape(changeNumber)}</result>"
	@results += "<result name='Change Instance Id'>#{escape(changeId)}</result>"
    ...
    @results += "</results>"
      ...
      if @error_handling == "Raise Error"
        ...
        @results = "<results>"
		@results += "<result name='Change Number'></result>"
		@results += "<result name='Change Instance Id'></result>"
  ...
  #
  def get_template_id(change_template_name, token)
    puts('getting template id String') if @debug_logging_enabled
	query = URI.encode("'Template Name'=\"#{change_template_name}\"")
	route = @api_server + '/arsys/v1/entry/CHG:Template?q='+query;
	puts("template id route #{route}") if @debug_logging_enabled
    id_resource = RestClient::Resource.new(route)
	id_response = id_resource.get(:authorization => "AR-JWT "+token)
	id_parsed = JSON.parse(id_response)
	puts("template id response #{id_response}") if @debug_logging_enabled
	
    return nil if id_parsed["entries"][0].nil?
	
	templateId = id_parsed["entries"][0]["values"]["InstanceId"]
