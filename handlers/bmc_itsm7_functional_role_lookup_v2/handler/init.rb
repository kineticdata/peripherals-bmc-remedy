...
class BmcItsm7FunctionalRoleLookupV2
  ...
  qual = %|'Support Group ID' = "#{@parameters['group_id']}" AND 'FunctionalRole' = "#{@parameters['functional_role']}"|
   entry = @@remedy_forms['CTM:SupportGroupFuncRoleLookUp'].find_entries(
           :all,
		   :fields => ['Login ID'],
		   :conditions => [qual])
   ...
   if entry.nil?
      raise "Unable to find record for Functional Role"
    end
	...
	entry.each do |entry|
		if (entry['Login ID'])
			id_list << '<LoginId>'+ entry['Login ID'] +'</LoginId>'
		end
	end
