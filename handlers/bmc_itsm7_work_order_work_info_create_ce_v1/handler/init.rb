...
class BmcItsm7WorkOrderWorkInfoCreateCeV1
  ...
  # Creates a record on the WOI:WorkInfo with the @field_values hash.  If the
  ...
  def execute
    ...
    # @field_values.
	puts("Using attachment_input_type: #{@parameters['attachment_input_type']}") if @debug_logging_enabled
    ...
    # Retrieve the WOI:WorkOrder entry that will be associated with the WOI:WorkInfo
    # entry.  This entry is retrieved using the work_order_id parameter and the
    # request id of the entry is used.
    work_order_entry = @@remedy_forms_remote['WOI:WorkOrder'].find_entries(
      :single,
      :conditions => [%|'Work Order ID' = "#{@parameters['work_order_id']}"|],
      :fields     => []
    )
    if work_order_entry.nil?
      raise(%|Could not find entry on WOI:WorkOrder with 'Work Order ID'="#{@parameters['work_order_id']}"|)
    end
    
	# Log the final values that will be used to create the WOI:WorkInfo record.
    puts(format_hash("Field Values:", @field_values)) if @debug_logging_enabled

    # Create the WOI:WorkInfo record using the @field_values hash that was built
	...
	entry = @@remedy_forms_remote['WOI:WorkInfo'].create_entry!(
  ...
  # Defines a hash of Remedy field names (for the 'WOI:WorkOrderInterface_Create'
  ...
  ##############################################################################
  # BMC ITSM7 Work Order Work Info Create handler utility functions
  ##############################################################################
  ...
  def create_attachment_from_field(field_name)
	puts("Creating attachment from Field: \n#{field_name}") if @debug_logging_enabled
...
#              file_info['link'] + "/url"
 ...
 def question_answers_string(submission_id)
      ...
      api_route = @info_values['api_server'] + '/' + @space_slug + 
                  ...
                  '/?include=values'
      puts "Get Question/Answers API ROUTE: #{api_route}"
