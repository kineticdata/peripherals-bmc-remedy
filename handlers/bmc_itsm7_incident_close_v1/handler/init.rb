...
class BmcItsm7IncidentCloseV1
  ...
  # Finds an ITSM Incident in the HPD:Help Desk form.
  #
  # This is a required method that is automatically called by the Kinetic Task
  # Engine.
  #
  ...
  def execute()
	incident_number = @parameters['incident_number']
    # Find the incident in the HPD:Help Desk form.  
	# Modify the record with the values provided.
	entry = @@remedy_forms['HPD:Help Desk'].find_entries(
      :single, :conditions => [%|'Incident Number' = "#{incident_number}"|]
    )
    raise %|Unable to find the Incident record for '#{incident_number}'.| if entry.nil?
