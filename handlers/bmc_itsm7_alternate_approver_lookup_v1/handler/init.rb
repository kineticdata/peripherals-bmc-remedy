...
class BmcItsm7AlternateApproverLookupV1
  ...
  qual = %|'For' = "#{@parameters['approver_id']}" AND 'Status' = "Current"|
   ...
   if entry.nil?
      id_list == '<Login_Ids></Login_Ids>'
	  results = <<-RESULTS
    <results>
	  <result name="Alternate Exists">No</result>
      <result name="Login IDs">#{escape(id_list)}</result>	  
    </results>
    RESULTS
	...
	entry.each do |entry|
		if (entry['Alternate'])
			id_list << '<LoginId>'+ entry['Alternate'] +'</LoginId>'
		end
	end
	...
	id_list << '</Login_Ids>'
 	...
 	if (id_list == '<Login_Ids></Login_Ids>')
		results = <<-RESULTS
    <results>
	  <result name="Alternate Exists">No</result>
      <result name="Login IDs">#{escape(id_list)}</result>	  
    ...
    results = <<-RESULTS
    <results>
	  <result name="Alternate Exists">Yes</result>
      <result name="Login IDs">#{escape(id_list)}</result>	  
    </results>
    RESULTS
    puts("Results: \n#{results}") if @debug_logging_enabled
